...
class tabdemo(QTabWidget):
    def __init__(self, parent=None):
        ...
        self.addTab(self.tab1, "Computing")
        self.addTab(self.tab2, "Table")
        self.addTab(self.tab3, "Graph")
        self.tab1UI()
        self.tab2UI()
        self.tab3UI()
    ...
    def tab1UI(self):
        ...
        hbox = QHBoxLayout()
       # hbox.addSpacing(1000)
        hbox.addWidget(okButton)

        layout = QFormLayout()
        layout2 = QFormLayout()
        layout3 = QFormLayout()
        method_button = QHBoxLayout()
        testing_button = QHBoxLayout()
        start_button = QPushButton("Start computing")
        start_button.clicked.connect(self.onClickedStart)
        #start_button = QPushButton()

        rbHistogram = QRadioButton("Brightness histogram")
        rbHistogram.toggled.connect(self.updateLabel)
        rbHistogram.setChecked(True)

        method_button.addWidget(rbHistogram)

        rbTrainingSet = QRadioButton("Training set")
        rbTestSet = QRadioButton("Test set")
        rbTrainingSet.setChecked(True)
        rbDFT = QRadioButton("DFT")
        rbDCT = QRadioButton("DCT")
        rbScale = QRadioButton("Scale")
        rbGradient = QRadioButton("Gradient")
        rbParallel = QRadioButton("Parallel")
        bg1 = QButtonGroup(self)
        bg1.addButton(rbDFT)
        bg1.addButton(rbDCT)
        bg1.addButton(rbScale)
        bg1.addButton(rbGradient)
        """
"""
        method_button.addWidget(rbDFT)
        method_button.addWidget(rbDCT)
        method_button.addWidget(rbScale)
        method_button.addWidget(rbGradient)
        """
"""
        method_button.addWidget( bg1)
        layout.addRow(QLabel("Method:"),method_button)
        testing_button.addWidget(rbTrainingSet)
        testing_button.addWidget(rbTestSet)
        layout2.addRow(QLabel("Testing:"), testing_button)

       # layout3.addRow(start_button)
...
"""
imgplot = plt.imshow(positive_patches[10])
#plt.show()


image = faces.images[0]
image2 = positive_patches[10]

histogram, bin_edges = np.histogram(faces.images[0], bins=256, range=(0, 5))
print(histogram)

histogram2, bin_edges2 = np.histogram(faces.images[1], bins=256, range=(0, 5))
print(histogram2)

featured_data = histogram

def distance(el1, el2):
	return np.linalg.norm(np.array(el1) - np.array(el2))

print(distance(histogram, histogram2))
...
"""
print(target_names[y_test[i]].rsplit(" ", 1)[-1])

number_test = 7
histogram, bin_edges = np.histogram(X[number_test], bins=256, range=(0, 100))
print(histogram)

histogram2, bin_edges2 = np.histogram(X[0], bins=256, range=(0, 100))
print(histogram2)
...
while i<100:
    i = i + 1
    histogram2, bin_edges2 = np.histogram(X[i], bins=256, range=(0, 100))
    if (min > distance(histogram, histogram2) and i != number_test):
            min = distance(histogram, histogram2)
            ind = i



print(y_test[ind], ind)
print("True: ", y[number_test])
histogram2, bin_edges2 = np.histogram(X[ind], bins=256, range=(0, 5))
print(distance(histogram, histogram2))
...
"""
X_train, X_test, y_train, y_test = methods.preparing_data()
n = 0
n_for_test = 50
#dct, number_test, data, true_target, targets
for j in range(n_for_test):
    dct = methods.get_dct(X_test[j])
    if (methods.classif(dct, j, X_train, y_test[j], y_train) == 1): n = n + 1
